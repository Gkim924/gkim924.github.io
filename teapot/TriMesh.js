 ...
 */
    ...
    */
    computeAABB(){
        ...
        for(var i=0;i<this.vBuffer.length;i+=3){
            ...
            if(this.vBuffer[x]>this.maxXYZ[0]){
                this.maxXYZ[0] = this.vBuffer[x];
            }
            if(this.vBuffer[y]>this.maxXYZ[1]) {
                this.maxXYZ[1] = this.vBuffer[y];
            }
            if(this.vBuffer[z]>this.maxXYZ[2]){
                this.mazXYZ[2] = this.vBuffer[z];
            }
            if(this.vBuffer[x]<this.minXYZ[0]) {
                this.minXYZ[0] = this.vBuffer[x];
            }
            if(this.vBuffer[y]<this.minXYZ[1]){
                this.minXYZ[1] = this.vBuffer[y];
            }
            if(this.vBuffer[z]<this.minXYZ[2]) {
                this.minXYZ[2] = this.vBuffer[z];
            }
        }

        console.log("--AABB Computed--");
       // console.log("maxXYZ:"+this.maxXYZ[0]+","+this.maxXYZ[1]+","+this.maxXYZ[2] +" MinXYZ:"+ this.minXYZ[0]+","+this.minXYZ[1]+","+this.minXYZ[2]);
    ...
    */
    loadFromOBJ(fileText)
    {
        ...
        //split over new line chars and parse line by line 
    
        //Your code here
        var text = fileText.split('\n');
        text.forEach(line => {

            var parsedLine = line.split(/\b\s+(?!$)/);

            if(parsedLine[0] == 'v'){
                this.vBuffer.push(parseFloat(parsedLine[1]),parseFloat(parsedLine[2]),parseFloat(parsedLine[3]));
                this.numVertices+=1;
            }

            if(parsedLine[0] == 'f'){
                this.fBuffer.push(parseInt(parsedLine[1]-1),parseInt(parsedLine[2]-1),parseInt(parsedLine[3]-1));
                this.numFaces+=1;
            }
